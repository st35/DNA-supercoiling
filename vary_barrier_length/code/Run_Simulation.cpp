...
void Run_Simulation(double promoterarg, int promoterid, double topoarg, int topoid, double barrierarg, int barrierid, int world_rank)
{
	barrier = barrierarg*0.34;
	...
	for(int i = 0; i < 1; i++)
	{
		outputfolder = "outputfiles/RUN_" + std::to_string(promoterid) + "_" + std::to_string(barrierid);
	...
	for(int i = 0; i < 1; i++)
	{
		outputfolder = "outputfiles/Linear_RUN_" + std::to_string(promoterid) + "_" + std::to_string(barrierid);
		runid = 16*world_rank + i;
		Gillespie_Simulation(force, gene_length0, gene_length1, spacer, config, clamp0_flag, clamp1_flag, T, controlgene, promoter0, promoter1, topoisomerase, stepwiseflag, runid, outputfolder, fileflag);
	}
...
int main(int argc, char *argv[])
{
	...
	double topoarg = 1.0;
	int topoid = 0;
	double barrierarg = std::stod(argv[3]);
	int barrierid = std::stoi(argv[4]);

	Run_Simulation(promoterarg, promoterid, topoarg, topoid, barrierarg, barrierid, world_rank);
