...
void Gillespie_Simulation(double force, double gene_length0, double gene_length1, double spacer, int config, int clamp0_flag, int clamp1_flag, double T, int controlgene, double promoter0, double promoter1, double topoisomerase, int stepwiseflag, int argid, std::string outputfolder, int fileflag, int switchflag, int passcount)
{
	...
	int Total_PolII_Count = 0, finish_count = 0, numevents = 4, event, check_flag = 0, orig_count = 0, zero_flag = 0, activeswitch = 0;
	std::vector<double> R;
	...
	double t = 0.0, dt, a0, p0, p1, sum_new, sum_prev, ini_phi, sub_t, sub_dt, epsilon = 1.0e-12;
	int counts_sum = 0, index;

	std::ofstream segment_file, sigma_file, torque_file, phi_file, x_file, v_file, av_file;
	if(fileflag == 1)
	{
	...
	while(out_times[controlgene].size() < T)
	{
		...
		if(config == 0 && event < Gene_Lengths.size() && activeswitch == 0) // Tandem genes
		{
			for(int i = 0; i < Gene_Lengths.size(); i++)
			{
				if(event != i)
		...
		else if(config == 1 && event < Gene_Lengths.size() && activeswitch == 0) // Divergent genes
		{
			for(int i = 0; i < Gene_Lengths.size(); i++)
			{
				if(event != i)
		...
		else if(config == 2 && event < Gene_Lengths.size() && activeswitch == 0) // Convergent genes
		{
			for(int i = 0; i < Gene_Lengths.size(); i++)
			{
				if(event != i)
		...
		else if(event < Gene_Lengths.size() && activeswitch == 0)
		{
		...
		if(switchflag == 1 && in_times[controlgene].size() == passcount)
		{
			activeswitch = 1;
		}
		if(out_times[controlgene].size() >= passcount)
		{
			break;
		}
	}
