...
void Run_Simulation(double promoterarg, int promoterid, double topoarg, int topoid, int world_rank)
{
	double force = 1.0;
	...
	for(int i = 0; i < 1; i++)
	{
		outputfolder = "outputfiles/RUN_" + std::to_string(promoterid) + "_" + std::to_string(topoid);
	...
	for(int i = 0; i < 1; i++)
	{
		outputfolder = "outputfiles/Linear_RUN_" + std::to_string(promoterid) + "_" + std::to_string(topoid);
		runid = 16*world_rank + i;
		Gillespie_Simulation(force, gene_length0, gene_length1, spacer, config, clamp0_flag, clamp1_flag, T, controlgene, promoter0, promoter1, topoisomerase, stepwiseflag, runid, outputfolder, fileflag);
	}
...
int main(int argc, char *argv[])
{
	...
	double topoarg = std::stod(argv[3]);
	int topoid = std::stoi(argv[4]);

	Run_Simulation(promoterarg, promoterid, topoarg, topoid, world_rank);
