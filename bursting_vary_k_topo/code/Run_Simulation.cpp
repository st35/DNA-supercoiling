...
void Run_Simulation(double topoisomerasearg, int argid, int world_rank)
{
	double force = 1.0;
	...
	clamp0_flag = 1;
	clamp1_flag = 1;
	topoisomerase = topoisomerasearg;
...
int main(int argc, char *argv[])
{
	...
	double topoisomerasearg = std::stod(argv[1]);
	int argid = std::stod(argv[2]);

	Run_Simulation(topoisomerasearg, argid, world_rank);
