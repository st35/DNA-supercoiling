...
void Gillespie_Simulation(double force, double gene_length0, double gene_length1, double spacer, int config, int clamp0_flag, int clamp1_flag, double T, int controlgene, double promoter0, double promoter1, double topoisomerase, int stepwiseflag, int argid, std::string outputfolder, int fileflag)
{
	...
	int Total_PolII_Count = 0, finish_count = 0, numevents = 4, event, check_flag = 0, orig_count = 0, zero_flag = 0, RNA_count = 0;
	std::vector<double> R;
	...
	std::ofstream segment_file, sigma_file, torque_file, phi_file, x_file, v_file, av_file, density_file, RNA_file, burst_file;
	if(fileflag == 1)
	{
	...
	burst_file.open(outputfolder + "/Burst_file_" + std::to_string(argid) + ".log");
		...
		if(Total_PolII_Count > 0)
		{
			...
			while(sub_t < t && Total_PolII_Count > 0)
			{
				...
				if(finish_count > 0)
				{
					...
					last_finish = sub_t;
					RNA_count += finish_count;
					burst_file << sub_t << " " << RNA_count << "\n";
				}
