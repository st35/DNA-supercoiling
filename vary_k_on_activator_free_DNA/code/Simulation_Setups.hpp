...
void Gillespie_Simulation(double force, double gene_length0, double gene_length1, double spacer, int config, int clamp0_flag, int clamp1_flag, double T, int controlgene, double promoter0, double promoter1, double topoisomerase, double TF, int stepwiseflag, int argid, std::string outputfolder, int fileflag)
{
	...
	double k_on_TF = (0.5 / 60.0)*TF;
	double k_off_TF = (5.0 / 60.0);
	...
	std::vector<int> Gene_Direction, PolII_Direction, PolII_Count;
	...
	int Total_PolII_Count = 0, finish_count = 0, numevents = 6, event, check_flag = 0, orig_count = 0, zero_flag = 0, promoter_state = 0;
	std::vector<double> R;
	...
	double t = 0.0, dt, a0, p0, p1, sum_new, sum_prev, ini_phi, sub_t, sub_dt, epsilon = 1.0e-12, last_finish_time = 0.0;
	int counts_sum = 0, index, switchflag = 0;

	std::ofstream segment_file, sigma_file, torque_file, phi_file, x_file, v_file, av_file, density_file, promoter_file, RNA_file;
	if(fileflag == 1)
	{
		...
		promoter_file.open(outputfolder + "/promoter_" + std::to_string(argid) + ".log");
	}
	density_file.open(outputfolder + "/density_file_" + std::to_string(argid) + ".log");
	RNA_file.open(outputfolder + "/RNA_file_" + std::to_string(argid) + ".log");
	...
	while(out_times[controlgene].size() < T)
	{
		...
		if(Total_PolII_Count > 0)
		{
			...
			while(sub_t < t && Total_PolII_Count > 0)
			{
				...
				if(fileflag == 1)
				{
					...
					v_file << sub_t << " ";
					av_file << sub_t << " ";
					promoter_file << sub_t << " " << promoter_state << "\n";
				...
				if(finish_count > 0)
				{
					RNA_file << ((double) finish_count) / (sub_t - last_finish_time) << "\n";
					last_finish_time = sub_t;
				}
		...
		if(event != 0 && event != 1 && event != 3 && event != 4 && event != 5)
		{
		...
		else if(event == Gene_Lengths.size() + 2)
		{
			if(promoter_state == 1)
			{
				std::cout << "The Great State of Vermont." << "\n";
			}
			promoter_state = 1;
		}
		else if(event == Gene_Lengths.size() + 3)
		{
			if(promoter_state == 0)
			{
				std::cout << "University of Hawaii at Manoa." << "\n";
			}
			promoter_state = 0;
		}
		density_file << Total_PolII_Count << "\n";
		check_flag = 0;
