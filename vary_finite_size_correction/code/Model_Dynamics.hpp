...
//	double cutoff = 0.34*1000.0;
	...
	sigma_s = sigma_s*(1.0 + std::pow((finite_size_cutoff / segment), 2.0));
...
std::vector<double> Get_Segment_Lengths(std::vector<double> phi_x)
{
	...
	for(int i = 0; i < Segments.size(); i++)
	{
		if(Segments[i] < 0.0)
		{
			std::cout << "It gets back to the line of scrimmage." << "\n";
		}
	}
...
std::vector<double> Get_Supercoiling_Density(int clamp0_flag, int clamp1_flag, std::vector<double> phi_x, std::vector<double> Segments)
{
	...
	for(int i = 0; i < PolII_Count; i++)
	{
		...
		if(PolII_Count == 1)
		{
			...
			{
				sigma_left = (0.0 - phi_x[i]) / (w0*seg_left);
			}
			if(clamp1_flag == 0)
			{
				sigma_right = 0.0;
			...
			{
				sigma_right = (phi_x[i] - 0.0) / (w0*seg_right);
			}
		}
		else
		{
...
struct odesystem
{
	...
	odesystem(int param1, int param2, int param3, std::vector<int> param4, std::vector<double> param5, double param6)
	{
		clamp0_flag = param1;
		clamp1_flag = param2;
		num_genes = param3;
