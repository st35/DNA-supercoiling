...
void Gillespie_Simulation(double force, double gene_length0, double gene_length1, double spacer, int config, int clamp0_flag, int clamp1_flag, double T, int controlgene, double promoter0, double promoter1, double topoisomerase, int stepwiseflag, int argid, std::string outputfolder, int fileflag)
{
	...
	int Total_PolII_Count = 0, finish_count = 0, numevents = 4, event, check_flag = 0, orig_count = 0, zero_flag = 0;
	std::vector<double> R;
	...
	double t = 0.0, dt, a0, p0, p1, sum_new, sum_prev, ini_phi, sub_t, sub_dt, epsilon = 1.0e-12, last_finish = 0.0;
	int counts_sum = 0, index, switchflag = 0, overall_sigma_flag = 0;

	std::ofstream segment_file, sigma_file, torque_file, phi_file, x_file, v_file, av_file, density_file, RNA_file, overall_sigma_file;
	if(fileflag == 1)
	{
	...
	overall_sigma_file.open(outputfolder + "/overall_sigma_file_" + std::to_string(argid) + ".log");
	...
	while(out_times[controlgene].size() < T)
	{
		...
		if(Total_PolII_Count > 0)
		{
			...
			while(sub_t < t && Total_PolII_Count > 0)
			{
				...
				if(overall_sigma_flag == 1)
				{
					overall_sigma_flag = 0;
					for(int i = 0; i < Sigma.size(); i++)
					{
						overall_sigma_file << Sigma[i] << " ";
					}
					overall_sigma_file << "\n";
				}
				...
				if(finish_count > 0)
				{
					RNA_file << ((double) finish_count) / (sub_t - last_finish) << "\n";
					last_finish = sub_t;
				}
